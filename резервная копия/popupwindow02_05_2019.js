	...
	$(document).ready(function(){
		...
		function GetSelectInfo(){
			$(".modal-content").find("select").each(function(index){
				...
				arrListOfDivForSelect[name_of_property]=carent_index;
		...
		function GetHideInfo(){
			$(".modal-content").find(".JSjumps, .JSspins, .JSsteps").each(function(index){
				...
				arrListOfDivForDisplay[name_of_property]=carent_value;
			});
			return;
		};//КОНЕЦ GetHideInfo()
		
		// функция конфигурирования всплывающего окна в соответствии со сделанным ранее выбором
		function ConfigurationOfPopUpWindow(){
			name_of_property=Iam_contanerTransparent_forSetting;
			name_of_property=name_of_property+0;
			if(arrListOfDivForSelect[name_of_property]!=undefined){
				$(".modal-content").find("select").each(function(index){
					name_of_property=Iam_contanerTransparent_forSetting;
					name_of_property=name_of_property+index;
					index_of_selected=arrListOfDivForSelect[name_of_property];
					$(this).prop("selectedIndex", index_of_selected)
					//симуляция изменения значения селекта
					if(index_of_selected>0){
						Iam_select_for_simulation=$(this);
						SimulateOfChangeSelect(Iam_select_for_simulation);
					};
				}); 
				$(".modal-content").find(".JSjumps, .JSspins, .JSsteps").each(function(index){
					name_of_property=Iam_contanerTransparent_forSetting;
					name_of_property=name_of_property+index;
					value_of_display=arrListOfDivForDisplay[name_of_property];
		...
		function SimulateOfChangeSelect(Iam_select_for_simulation){
			Iam_select_for_simulation.change();
			return;
		};
